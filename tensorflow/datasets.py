...
def random_crop_and_pad(image, output_size=(224, 224)):
    pre_crop_size = (output_size[0]+32, output_size[1]+32)
    image = tf.image.resize(image, pre_crop_size, preserve_aspect_ratio=True)
    image = tf.image.resize_with_pad(image, *pre_crop_size)
    image = tf.image.random_crop(image, (*output_size, 3))
    return image
...
def preprocess(record):
    ...
    image = random_crop_and_pad(image)
    image = (image - _CHANNEL_MEANS) / _CHANNEL_STDS
    label = tf.cast(label, tf.int32)
    return image, label

def restore_image(image):
    return (image * _CHANNEL_STDS + _CHANNEL_MEANS) / 255.

car_labels = {0: 'AM General Hummer SUV 2000',
